...
func (e *Editor) processKey(gtx layout.Context) (EditorEvent, bool) {
	...
	for {
		...
		case transfer.DataEvent:
			if resEvt := e.onPasteEvent(ke); resEvt != nil {
				return resEvt, true
			}
		case key.Event:
			...
			idx := matchedCmd(ke, allCmds)
			if idx >= 0 {
				cmd := allCmds[idx]
				resEvt := cmd.handler(gtx, ke)
				if resEvt != nil {
					if isChangeEvent(resEvt) {
						e.text.Changed()
					}
					return resEvt, true
				}
